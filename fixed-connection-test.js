...
async function testDirectConnection() {
  ...
  try {
    ...
    if (collections.notifications) {
      ...
      // Check for Pedro's notifications
      const pedroNotifications = await collections.notifications.find({ userId: pedro._id }).sort({ createdAt: -1 }).limit(5);
      console.log(`Pedro has ${pedroNotifications.length} notifications`);
      
      if (pedroNotifications.length > 0) {
        console.log('Pedro\'s recent notifications:');
        pedroNotifications.forEach((notif, index) => {
          console.log(`  ${index + 1}. ${notif.title} - ${notif.message}`);
        });
      } else {
        console.log('Pedro has no notifications');
      }
    }
